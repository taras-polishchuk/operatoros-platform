# Technical Debt Register — v1.0.0

Carried forward from v1.0.0-rc1 (`artifacts/release-candidates/rc1/technical-debt.md`).
No new technical debt was introduced by the v1.0 public-readiness work
(landing page, docs, API reference, release bundle). v1.0.0 status is:

- **TD-001..TD-006** — **unchanged**, carried forward verbatim.
- **TD-007** — **new**: this file itself serves as the carry-forward record;
  for v1.1 the technical-debt register should be promoted to a top-level
  `artifacts/TECHNICAL-DEBT.md` and removed from per-release folders to
  avoid drift between rc1 and v1.0 copies.
...
## TD-007: Technical-debt register is duplicated across release folders

**Impact:** `technical-debt.md` is now present in both `artifacts/release-candidates/rc1/` and `artifacts/release-candidates/v1.0/`. Future releases will spawn further copies unless promoted.
**Mitigation:** Carry-forward is explicit and itemized at the top of this file.
**Disposition:** Promote to a top-level `artifacts/TECHNICAL-DEBT.md` in v1.1; per-release folders reference it.
