...
describe("validatePathInside helper", () => {
  describe("rejects traversal attacks", () => {
    ...
    it("blocks ../../escape", () => {
      expect(() => validatePathInside("/var/data", "/var/data/../../etc/shadow")).toThrow("PATH_OUTSIDE_BASE");
    });
    it("blocks /etc/passwd (direct absolute path outside base)", () => {
      expect(() => validatePathInside("/home/user/snapshots", "/etc/passwd")).toThrow("PATH_OUTSIDE_BASE");
    });
    ...
    it("blocks /var/log/auth.log (looks similar but not same base)", () => {
      expect(() => validatePathInside("/var/data", "/var/log/auth.log")).toThrow("PATH_OUTSIDE_BASE");
    });
  });
